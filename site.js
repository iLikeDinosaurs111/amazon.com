...
function updateMeltingSnowmanGraphic() {
  ...
  meltingSnowmanContainer.innerHTML = `<img src="./images/MeltingSnowman${imageCount}.gif" alt="MeltingSnowman ${imageCount}" width=100% height=100%> <h2>You have ${7-imageCount} tries left.</h2>`;
  imageCount++;
}
...
function checkWinOrLose() {
  if (guessedLetters.join('') === wordToGuess) {
    ...
    const meltingSnowmanContainer = document.querySelector('.MeltingSnowman');
    meltingSnowmanContainer.innerHTML = `<img src="./images/gameover.gif" alt="gameover" width=100% height=100%>`;
    const letterButtons = document.querySelectorAll('.letters button');
    letterButtons.forEach(button => {
      button.disabled = true;
      button.removeEventListener('click', handleGuess);
